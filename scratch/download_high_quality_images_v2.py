    ...
    try:
        ...
        if infobox_match:
            ...
            if img_matches:
                ...
                # Return the thumbnail URL directly! Do NOT strip it.
                return img_url
    except Exception as e:
        pass
    return None
...
def process_product(product):
    ...
    print(f"[RETRY-START] Processing: '{name}' ({category})...")
    ...
    if img_url:
        ...
        if bytes_saved > 0:
            print(f"[RETRY-SUCCESS] '{name}' ({category}) -> {filename} ({bytes_saved} bytes) from {source}")
            return prod_id, filename
            
    print(f"[RETRY-FAILED] Could not download image for '{name}'")
    return prod_id, None
